	...
	while board[n-1][n-1] == 0:
		...
		for h,t in zip(head,tail):
			...
			for y,x in zip(movey,movex): # move up->right->down->left
				if 0 <= x+h[1] < n and 0 <= y+h[0] < n and 0 <= x+t[1] < n and 0 <= y+t[0] < n:
			...
			if h[0] == t[0]:
				#spin_down
				if 0 <= h[0]+1 < n and 0 <= t[0]+1 < n and board[h[0]+1][h[1]] == (cnt or 0) and board[t[0]+1][t[1]] == (cnt or 0):
			...
			#spin_col
				...
				#spin_right
				if (0 <= h[1]+1 < n and 0 <= t[1]+1 < n) and board[h[0]][h[1]+1] == (cnt or 0) and board[t[0]][t[1]+1] == (cnt or 0):
					new_head.append([h[0],h[1]])
